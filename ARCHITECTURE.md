...
5. `/` 流派分析（僅 model）：版圖吃當期熱門榜 Top50 快照按 `task` 分組；趨勢吃 `MetricHistory`（`sortBy=trendingScore`，每輪各 task likes 佔比以前 3 輪平均=100 指數化畫折線，缺席輪次斷線、Top5＋其他；佔比本身幾乎不動故不用堆疊；`task` 欄 2026-09-23 起才寫，舊輪跳過）
